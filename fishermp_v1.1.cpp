...
typedef pair<pair<int,int>,double> iidPAIR;
typedef map<vector<string>,vector<int> > vectstrTOvectintHash;
...
int main(int argc, const char** argv){
	...
	int MotifNumber=0;//number of motifs to find
	int CombinedMotifs=2;//combined number
	...
	if(argc!=2 && argc!=4 && argc!=6 && argc!=8 && argc!=10 && argc!=12 && argc!=14){
		...
		cout<<"-c\t\tdefine the number of cooperate motifs(default=2, you can select 1,2,3 for single motif, pair motifs, or triple motifs)\n";
		cout<<"-n\t\tNumber of motifs to find(default=all possible motifs)\n";
		cout<<"-t\t\tnumber of threads to call(default="<<ThreadNum<<" i.e. =M-m+1)\n";
		cout<<"\n*******\nVersion:1.1 (Dec 10,2018)\n\n";
		cout<<"For example: \n"<<argv[0]<<" foregroundfile.fasta -b backgroundfile.fasta -m 5 -M 9 -n 5 -t 10  > output.file\n\n";
		cout<<endl;
		exit(1);
	}
	...
	for(int i=2;i<argc-1;i=i+2){
		...
		if(strindex=="-b"){
			...
			string scmotifs(argv[i+1]);
			istringstream iscmotifs(scmotifs);
			iscmotifs>>CombinedMotifs;
			if(CombinedMotifs<=0){
				cout<<"ERROR: the wrong parameter of '-c', please input a positive ineger(1,2,or, 3)."<<endl; exit(1);
			}
		}else if(strindex=="-t"){
...
//=================end of first-hand motif forming ==============================
	...
	vectstrTOvectintHash motif_foreseq_labelvect_hash;//motif seqs to foreground seq labels
	vectstrTOvectintHash motif_backseq_labelvect_hash;//motif seqs to background seq labels
	...
	for(int i=0;i<motif_matrix.size();i++){
		...
		vector<int> final_mergedLineLabelVect;
		vector<int> final_mergedBackLineVect;
		...
		for(int j=1;j<mv.size();j++){
			...
			if(p_value<0.05){
				...
				ma=a;
				mc=c;
				final_mergedBackLineVect=mergedBackLineVect;
				final_mergedLineLabelVect=mergedLineLabelVect;
			}else{
				break;
			}
		}
		...
		motif_foreseq_labelvect_hash.insert(vectstrTOvectintHash::value_type(oneMotifKmerVect,final_mergedLineLabelVect));
		motif_backseq_labelvect_hash.insert(vectstrTOvectintHash::value_type(oneMotifKmerVect,final_mergedBackLineVect));
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~compute motif pair p-value~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	if(MotifNumber==0){
		MotifNumber=motifpvalue_pair_vect.size();
	}
	if(CombinedMotifs == 2){
		...
		for(int mt=0;mt<MotifNumber;mt++){
			...
			for(int mk=mt+1;mk<MotifNumber;mk++){
				...
				if(line_count<5000){
					...
					a=a_intersect.size()/10;
					b=(line_count-a)/10;
					c=c_intersect.size()/10;
					d=(back_line_count-c)/10;
				}
				p_value=exp(log_combin(a+c, a)+log_combin(b+d,b)-log_combin(a+b+c+d,a+b));
