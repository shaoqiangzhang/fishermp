...
int main(int argc, const char** argv){
	...
	if(argc!=2 && argc!=4 && argc!=6 && argc!=8 && argc!=10 && argc!=12){
		...
		cout<<"\n*******\nDesigned by Shaoqiang Zhang (zhangshaoqiang@tjnu.edu.cn), Feb/02/2018\n\n";
...
//===end==read the background file=====================
	...
	omp_set_num_threads(ThreadNum);
	#pragma omp parallel default(shared) private(i,p_value, a, b, c, d, lineLabelVect,kpvector_private)
	{
...
//===The following is to form first-hand motif by computing difference between kmers======
	...
	MatrixStr matrix_private;
	int sdv=sdVector.size();
	omp_set_num_threads(ThreadNum);
	#pragma omp parallel for default(none) private(matrix_private) shared(motif_matrix,sdVector,sdv)
	for (int i=0;i<100;i++){
			...
			matrix_private.push_back(oneMotifKmerVect);
			#pragma omp critical (d)
			motif_matrix.insert(motif_matrix.end(),matrix_private.begin(),matrix_private.end());
	}
...
//=================end of first-hand motif forming ==============================
	...
	vector<VectDoublePair> vdpair_vector_private;
	VectDoublePair vdpair;
	
	vector<strVectIntPair> ma_pairvect;
	vector<strVectIntPair> ma_pair_vect_private;
	vector<strVectIntPair> mc_pairvect;
	vector<strVectIntPair> mc_pair_vect_private;
	strVectIntPair ma_pair;
	strVectIntPair mc_pair;
	
	omp_set_num_threads(ThreadNum);
	#pragma omp parallel for default(shared) \
	private(a,b,c,d,p_value,vdpair,vdpair_vector_private, ma_pair,ma_pair_vect_private,mc_pair,mc_pair_vect_private)
	for(int i=0;i<motif_matrix.size();i++){
		...
		vdpair=make_pair(oneMotifKmerVect, merged_Pvalue);
		vdpair_vector_private.push_back(vdpair);
		
		ma_pair=make_pair(oneMotifKmerVect, ma);
		ma_pair_vect_private.push_back(ma_pair);
		
		mc_pair=make_pair(oneMotifKmerVect, mc);
		mc_pair_vect_private.push_back(mc_pair);
		#pragma omp critical (e) 
		{
			motifpvalue_pair_vect.insert(motifpvalue_pair_vect.end(),vdpair_vector_private.begin(),vdpair_vector_private.end());
			ma_pairvect.insert(ma_pairvect.end(),ma_pair_vect_private.begin(),ma_pair_vect_private.end());
			mc_pairvect.insert(mc_pairvect.end(),mc_pair_vect_private.begin(),mc_pair_vect_private.end());
		}
	}
	...
	for(int i=0;i<ma_pairvect.size();i++){
		strvect_ma_hash.insert(VectIntHash::value_type(ma_pairvect[i].first, ma_pairvect[i].second));
		strvect_mc_hash.insert(VectIntHash::value_type(mc_pairvect[i].first, mc_pairvect[i].second));
	}
